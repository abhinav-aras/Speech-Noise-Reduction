...
filtered_n = filtered_n * (rms(clean_n)/rms(filtered_n));

noise_in = noisy_n - clean_n;
...
results = struct('snr_in_dB', snr_in, ...
                 'snr_out_dB', snr_out, ...
...
subplot(3,1,2);
plot(t, noisy_n);
title(sprintf('Noisy Signal (SNR = %.2f dB)', snr_in));
xlabel('Time (s)');
ylabel('Amplitude');

subplot(3,1,3);
plot(t, filtered_n);
title(sprintf('Filtered Signal (SNR = %.2f dB)', snr_out));
xlabel('Time (s)');
ylabel('Amplitude');

figure;
subplot(3,1,1);
spectrogram(clean_n,256,[],[],fs,'yaxis');
title('Clean Signal Spectrogram');

subplot(3,1,2);
spectrogram(noisy_n,256,[],[],fs,'yaxis');
title('Noisy Signal Spectrogram');

subplot(3,1,3);
spectrogram(filtered_n,256,[],[],fs,'yaxis');
title('Filtered Signal Spectrogram');

fprintf('\n--- Playing CLEAN Audio ---\n');
soundsc(clean_n, fs);
pause(length(clean_n)/fs + 1);

fprintf('\n--- Playing NOISY Audio ---\n');
soundsc(noisy_n, fs);
pause(length(noisy_n)/fs + 1);

fprintf('\n--- Playing FILTERED Audio ---\n');
soundsc(filtered_n, fs);
